...
CHALLENGE_BASE_URL = 'https://challenge.authsignal.com'
...
class Client(object):
    ...
    def __init__(
        ...
        Args:
            ...
            tenant_id: Your Authsignal Tenant ID
        ...
        if tenant_id is None:
            tenant_id = authsignal.tenant_id
    ...
    def track(self, user_id, action, payload=None, path=None):
        ...
        try:
            response = self.session.post(
                path,
                data=json.dumps(
                    payload if payload is not None else {}, cls=DecimalEncoder),
    ...
    def validate_challenge(self, token, user_id=None):
        try:
            audience = f"{CHALLENGE_BASE_URL}/{self.tenant_id}"
        ...
        if action and idempotency_key:
            action_result = self.get_action(
                user_id=decoded_user_id, action=action, idempotency_key=idempotency_key)
